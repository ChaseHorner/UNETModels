...
def test_decoder_basic_scale_two():
    decoder = Decoder(64, 3, scale_size = 2)
...
def test_decoder_start():
    ...
    decoder = Decoder(in_channels, out_channels, scale_size = 2)
...
def test_decoder_end():
    ...
    decoder = Decoder(in_channels, out_channels, skip_channels=skip_channels, scale_size = 2)
...
def test_decoder_chain_start():
    ...
    dec_1 = Decoder(in_channels_1, out_channels_1, skip_channels=skip_channels_1, scale_size=2)
    dec_2 = Decoder(in_channels_2, out_channels_2, skip_channels=skip_channels_2, scale_size=2)
...
def test_decoder_chain_full():
    dec_7 = Decoder(configs.C7 + configs.W1 + configs.W2, configs.C6, skip_channels=configs.C6)
    dec_6 = Decoder(configs.C6, configs.C5, skip_channels=configs.C5)
    dec_5 = Decoder(configs.C5, configs.C4, skip_channels=configs.C4)
    dec_4 = Decoder(configs.C4, configs.C3, skip_channels=configs.C3)
    dec_3 = Decoder(configs.C3, configs.C2 + configs.S1, skip_channels=configs.C2 + configs.S1)
